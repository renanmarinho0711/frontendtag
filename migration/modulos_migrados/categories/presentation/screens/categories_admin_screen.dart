...
class _CategoriasAdminScreenState extends ConsumerState<CategoriasAdminScreen>
  ...
  List<Map<String, dynamic>> _getCategorias() {
    ...
    return allCategories.map((cat) {
      // Contar subcategorias: categorias que têm esta como parentId
  ...
  // OTIMIZAÇÃO: Getter com cache - só recalcula se filtros mudaram
            ...
            headerSliverBuilder: (context, innerBoxIsScrolled) {
              return [
                SliverToBoxAdapter(
                  child: Column(
                    children: [
                      if (_isSelectMode) _buildBulkActions(context),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.edit_rounded, color: ThemeColors.of(context).surface),
            onPressed: _selectedItems.isEmpty ?  null : () => _bulkEdit(),
          ),
          IconButton(
            icon: const Icon(Icons.delete_rounded, color: ThemeColors.of(context).surface),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(AppSizes.paddingBase.get(isMobile, isTablet)),
                borderSide: BorderSide(color: ThemeColors.of(context).greenMaterial, width: 2),
              ),
              filled: true,
            ...
            children: [
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      _buildFilterChip(context, 'Todas', 'todas'),
                      const SizedBox(width: 8),
                      _buildFilterChip(context, 'Ativas', 'ativas'),
                      const SizedBox(width: 8),
                      _buildFilterChip(context, 'Inativas', 'inativas'),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildGridView() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildGridCard(context, _categoriasFiltradas[index], index);
      },
    );
  }
  ...
  Widget _buildListView() {
    return ListView.builder(
      ...
      itemCount: _categoriasFiltradas.length,
      itemBuilder: (context, index) {
        return _buildListCard(context, _categoriasFiltradas[index], index);
      },
    );
  }

  Widget _buildGridCard(BuildContext context, Map<String, dynamic> categoria, int index) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
  ...
  Widget _buildListCard(BuildContext context, Map<String, dynamic> categoria, int index) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
    final isSelected = _selectedItems.contains(categoria);
    ...
    return TweenAnimationBuilder(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isSelected
              ? Border.all(color: ThemeColors.of(context).greenMaterial, width: 2)
              : Border.all(color: ThemeColors.of(context).textSecondary),
          boxShadow: [
            BoxShadow(
              color: isSelected
                  ? ThemeColors.of(context).greenMaterial.withValues(alpha: 0.2)
                          ...
                          children: [
                            ...
                            if (! categoria['ativa'])
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: ThemeColors.of(context).redMain.withValues(alpha: 0.2),
                                  borderRadius: BorderRadius.circular(8),
                                  border: Border.all(color: ThemeColors.of(context).redMain),
                                ),
                                child: Text(
                                  'INATIVA',
                                  style: TextStyle(
                    ...
                    PopupMenuButton(
                      ...
                      itemBuilder: (context) => [
                        PopupMenuItem(
                          child: Row(
                            children: [
                              Icon(Icons.edit_rounded, size: 20, color: ThemeColors.of(context).blueMaterial),
                        ...
                        PopupMenuItem(
                          child: Row(
                            children: [
                              Icon(Icons.delete_rounded, size: 20, color: ThemeColors.of(context).redMain),
                              const SizedBox(width: 12),
                              const Text('Excluir'),
                            ],
                          ),
  ...
  void _bulkDelete() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        icon: Icon(Icons.delete_sweep_rounded, color: ThemeColors.of(context).redMain, size: 48),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('$deleted categorias excluídas'),
                  backgroundColor: ThemeColors.of(context).redMain,
                  behavior: SnackBarBehavior.floating,
                ),
              );
