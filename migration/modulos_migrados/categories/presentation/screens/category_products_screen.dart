...
class _CategoriasProdutosScreenState extends ConsumerState<CategoriasProdutosScreen>
          ...
          child: NestedScrollView(
            headerSliverBuilder: (context, innerBoxIsScrolled) {
              return [
                ...
                if (_isSelectMode) SliverToBoxAdapter(child: _buildBulkActions(context)),
              ];
            },
  ...
  Widget _buildStatsCards(BuildContext context) {
    ...
    final estatisticas = [
      {'label': 'Bebidas', 'valor': '247', 'icon': Icons.local_drink_rounded, 'cor': ThemeColors.of(context).blueMaterial, 'mudanca': '+12', 'tipo': 'aumento'},
      ...
      {'label': 'Perecíveis', 'valor': '189', 'icon': Icons.restaurant_rounded, 'cor': ThemeColors.of(context).greenMaterial, 'mudanca': '+7', 'tipo': 'aumento'},
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.drive_file_move_rounded, color: ThemeColors.of(context).surface),
            onPressed: _selectedProducts.isEmpty ?  null : () => _moveProducts(),
          ),
          IconButton(
            icon: const Icon(Icons.link_off_rounded, color: ThemeColors.of(context).surface),
  ...
  Widget _buildGridView() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildProductGridCard(context, _produtosFiltrados[index], index);
      },
    );
  }
  ...
  Widget _buildListView() {
    return ListView.builder(
      ...
      itemCount: _produtosFiltrados. length,
      itemBuilder: (context, index) {
        return _buildProductListCard(context, _produtosFiltrados[index], index);
      },
    );
  }

  Widget _buildProductGridCard(BuildContext context, Map<String, dynamic> produto, int index) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
  ...
  Widget _buildProductListCard(BuildContext context, Map<String, dynamic> produto, int index) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
    final isSelected = _selectedProducts.contains(produto);
    final categoria = _categorias.firstWhere((c) => c['nome'] == _categoriaSelecionada);
