...
/// - Passo 2: Preço (Preço de venda, Preço por kg, Custo)
/// - Passo 3: Confirmar (Resumo + opção de vincular tag)
class ProdutosAdicionarScreen extends ConsumerStatefulWidget {
  final VoidCallback? onBack;
  final ProductModel? initialProduct; // Para duplicação de produtos
...
class _ProdutosAdicionarScreenState extends ConsumerState<ProdutosAdicionarScreen> 
  ...
  List<String> get _categoriasFallback => ['Bebidas', 'Mercearia', 'Perecíveis', 'Limpeza', 'Higiene'];
  ...
  void initState() {
    ...
    // Inicializa com dados do produto inicial (duplicação)
    ...
    // Listeners para detectar alterações
    ...
    // Focus automático no primeiro campo e carrega categorias após build
    ...
    try {
      ...
      if (suggestedState.status == LoadingStatus.success && 
        ...
        setState(() {
          ...
          // Se a categoria atual não existir nas carregadas, define a primeira
      ...
      // Fallback silencioso - usa categorias padrão do CategoryThemes
  ...
  /// Constrói os chips de categoria usando dados do backend ou fallback
  List<Widget> _buildCategoriaChips(BuildContext context) {
  ...
  void _nextStep() {
    if (_currentStep < _totalSteps - 1) {
      if (_currentStep == 0 && !_step1Valid) {
        _showValidationError('Complete todos os campos obrigatórios do Passo 1');
        return;
      }
      if (_currentStep == 1 && !_step2Valid) {
        _showValidationError('Informe um preço válido maior que zero');
        return;
      }
  ...
  void _showValidationError(String message) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          children: [
            const Icon(Icons.warning_rounded, color: ThemeColors.of(context).surface),
            ...
            children: [
              ...
              Expanded(
                child: PageView(
                  ...
                  children: [
                    _buildStep1InformacoesBasicas(context),
                    _buildStep2Preco(context),
                    _buildStep3Confirmar(context),
                  ],
                ),
              ),
  ...
  Widget _buildProgressIndicator(BuildContext context) {
    ...
    final steps = ['Informações', 'Preço', 'Confirmar'];
            ...
            children: List.generate(_totalSteps * 2 - 1, (index) {
              if (index.isOdd) {
                ...
                return Container(
                  ...
                  child: Center(
                    child: isCompleted
                        ? const Icon(Icons.check_rounded, color: ThemeColors.of(context).surface, size: 16)
                        : Text(
                            '${stepIndex + 1}',
                            style: TextStyle(
  ...
  Widget _buildStep1InformacoesBasicas(BuildContext context) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Toque para abrir a câmera ou digite manualmente',
                ...
                children: [
                  ...
                  _categoriasLoading
                    ...
                    : Wrap(
                        ...
                        children: _buildCategoriaChips(context),
                      ),
  ...
  Widget _buildStep2Preco(BuildContext context) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildTextField(context, controller: _precoController,
                ...
                const SizedBox(height: AppSpacing.sm),
                Text(
                  'Este é o preço que será exibido na etiqueta ESL',
                  style: _responsiveTextStyle(AppTextStyles.caption.copyWith(
                    color: ThemeColors.of(context).textSecondary,
                  )),
                ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: AppSpacing.md),
                    Expanded(
                      child: _buildTextField(context, controller: _custoController,
  ...
  Widget _buildMargemInfo(BuildContext context) {
  ...
  Widget _buildStep3Confirmar(BuildContext context) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
        ...
        children: [
          Container(
            ...
            child: Column(
              children: [
                ...
                _buildResumoItem(context, 'Código de Barras', _codigoController.text, Icons.qr_code_rounded),
                _buildResumoItem(context, 'Nome', _nomeController.text, Icons.inventory_2_rounded),
                _buildResumoItem(context, 'Categoria', _categoria, categoryTheme.icon),
                _buildResumoItem(context, 'Preço', 'R\$ ${preco.toStringAsFixed(2)}', Icons.attach_money_rounded),
                if (precoKg != null)
                  _buildResumoItem(context, 'Preço/Kg', 'R\$ ${precoKg.toStringAsFixed(2)}', Icons.monitor_weight_rounded),
                if (_descricaoController.text.isNotEmpty)
                  _buildResumoItem(context, 'Descrição', _descricaoController.text, Icons.description_rounded),
              ],
            ),
          ),
          ...
          Container(
            ...
            child: CheckboxListTile(
              ...
              title: Text(
                'Vincular etiqueta ESL após criar',
                style: _responsiveTextStyle(AppTextStyles.body.copyWith(
                  fontWeight: FontWeight.w600,
                )),
              ),
              subtitle: Text(
                'Abrir tela de vinculação de tags automaticamente',
                style: _responsiveTextStyle(AppTextStyles.caption.copyWith(
                  color: ThemeColors.of(context).textSecondary,
                )),
              ),
          ...
          Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.info_outline_rounded, color: ThemeColors.of(context).blueMain),
                const SizedBox(width: AppSpacing.md),
                Expanded(
                  child: Text(
                    'Revise os dados antes de salvar. Você poderá editar o produto posteriormente.',
                    style: TextStyle(
                      fontSize: 13,
  ...
  Widget _buildResumoItem(BuildContext context, String label, String value, IconData icon) {
    return Padding(
  ...
  Widget _buildDiscardDialog(BuildContext context) {
    return AlertDialog(
      ...
      content: const Text(
        'Você tem dados não salvos. Deseja descartar todas as alterações?',
        textAlign: TextAlign.center,
      ),
      actions: [
        TextButton(
  ...
  void _abrirScanner() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          children: [
            const Icon(Icons.info_rounded, color: ThemeColors.of(context).surface),
            const SizedBox(width: AppSpacing.md),
            const Expanded(
    ...
    if (storeId == null || storeId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Row(
            children: [
              const Icon(Icons.error_rounded, color: ThemeColors.of(context).surface),
              const SizedBox(width: AppSpacing.md),
              Expanded(
                child: Text(
