  ...
  Future<void> loadProducts({bool refresh = false}) async {
    ...
    try {
      if (_repository != null) {
        ...
        // Repositório não disponível - mostrar erro
        state = state.copyWith(
          status: LoadingStatus.error,
          error: 'Repositório de produtos não configurado',
        );
      }
    } catch (e) {
      state = state.copyWith(
...
class ProductStatisticsNotifier extends StateNotifier<ProductStatisticsState> {
  ...
  Future<void> loadStatistics({String? storeId}) async {
    ...
    try {
      ...
      // Fallback: valores zerados se não houver repositório ou resposta
      state = state.copyWith(
        status: LoadingStatus.success,
  ...
  IconData _getCategoryIcon(String nome) {
    ...
    if (lower.contains('perecí') || lower.contains('pereci')) return CategoryThemes.pereciveis.icon;
  ...
  Color _getCategoryColor(String nome) {
    ...
    if (lower.contains('perecí') || lower.contains('pereci')) return CategoryThemes.pereciveis.color;
...
// O fromJson agora suporta tanto campos em português quanto em inglês
...
class StockItem {
  ...
  String get statusEstoque {
    ...
    if (emAlerta || estoqueAtual <= estoqueMinimo) return 'Crítico';
...
class StockNotifier extends StateNotifier<StockState> {
  ...
  Future<void> loadStock({String? storeId, String? status}) async {
    ...
    try {
      ...
      // Fallback: lista vazia se não houver repositório ou resposta
      state = state.copyWith(
        status: LoadingStatus.success,
        items: [],
      );
  ...
  Future<bool> confirmBinding() async {
    ...
    try {
      if (_tagsRepository != null && state.tagMacAddress != null && state.product != null) {
        ...
        state = state.copyWith(
          status: LoadingStatus.error,
          error: 'Repositório de tags não configurado ou dados incompletos',
        );
        return false;
      }
    } catch (e) {
...
/// Provider do Repository de Produtos (conectado à API)
final produtoRepositoryProvider = Provider<ProdutoRepository>((ref) {
  return ProdutoRepository();
});

/// Provider do Repository de Tags (conectado à API)
final tagsRepositoryProvider = Provider<TagsRepository>((ref) {
  return TagsRepository();
});

/// Provider de lista de produtos (Riverpod StateNotifier) - CONECTADO À API
...
/// Provider de estatísticas de produtos (Riverpod StateNotifier) - CONECTADO À API
...
/// Provider de vinculação Tag/QR - CONECTADO À API
