...
/// Detecta situações e sugere próximos passos automaticamente
class FluxosInteligentesCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Se não há fluxos, não mostrar o card
    if (fluxos.isEmpty) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            AppThemeColors.blueCyan.withValues(alpha: 0.05),
            AppThemeColors.blueLight.withValues(alpha: 0.08),
          ],
  ...
  List<_FluxoInteligente> _getFluxosPendentes(DashboardState state) {
    ...
    if (tagsNaoVinculadas > 0) {
      fluxos.add(_FluxoInteligente(
        ...
        titulo: '$tagsNaoVinculadas ${tagsNaoVinculadas == 1 ? "tag não vinculada" : "tags não vinculadas"}',
    ...
    if (importacoesComErro > 0) {
      fluxos.add(_FluxoInteligente(
        ...
        descricao: 'Alguns produtos não foram importados corretamente',
        acoes: [
