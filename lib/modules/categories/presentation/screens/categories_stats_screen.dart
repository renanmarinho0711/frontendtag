...
class _CategoriasEstatisticasScreenState extends ConsumerState<CategoriasEstatisticasScreen>
  ...
  Color _getColorForCategory(String nome) {
    final lower = nome.toLowerCase();
    if (lower.contains('bebida')) return ThemeColors.of(context).blueMaterial;
    if (lower.contains('mercear')) return ThemeColors.of(context).brownMain;
    if (lower.contains('perec')) return ThemeColors.of(context).greenMaterial;
  ...
  List<Color> _getGradientForCategory(String nome) {
    final lower = nome.toLowerCase();
    if (lower.contains('bebida')) return [ThemeColors.of(context).blueMaterial, ThemeColors.of(context).blueDark];
    if (lower.contains('mercear')) return [ThemeColors.of(context).brownMain, ThemeColors.of(context).brownDark];
    if (lower.contains('perec')) return [ThemeColors.of(context).greenMaterial, ThemeColors.of(context).successIcon];
    if (lower.contains('limp')) return [ThemeColors.of(context).blueCyan, ThemeColors.of(context).blueMaterial];
    if (lower.contains('higien')) return [ThemeColors.of(context).blueLight, ThemeColors.of(context).blueMaterial];
  ...
  Widget _buildPeriodSelector(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: _buildPeriodOption(context, '7 dias', '7dias')),
          Expanded(child: _buildPeriodOption(context, '30 dias', '30dias')),
          Expanded(child: _buildPeriodOption(context, '90 dias', '90dias')),
        ],
      ),
    );
  }
  ...
  Widget _buildPeriodOption(BuildContext context, String label, String value) {
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: isSelected
              ? LinearGradient(colors: [ThemeColors.of(context).greenMaterial, ThemeColors.of(context).greenDark])
              : null,
          borderRadius: BorderRadius.circular(AppSizes.paddingBase.get(isMobile, isTablet)),
        ),
        child: Text(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          _buildTopCategoriesCard(context),
        ],
      ),
    );
  }
  ...
  Widget _buildCategoriesTab() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildCategoryDetailCard(context, _categorias[index], index);
      },
    );
  }
  ...
  Widget _buildTrendsTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          _buildGrowthRankingCard(context),
          SizedBox(
            height: AppSizes.cardPadding.get(isMobile, isTablet),
          ),
  ...
  Widget _buildSummaryCards(BuildContext context) {
    ...
    final cards = [
      _buildSummaryCard(context, 'Total de Vendas',
      ...
      _buildSummaryCard(context, 'Faturamento',
        'R\$ ${_dadosAtuais['totalFaturamento'].toStringAsFixed(2)}',
        Icons.attach_money_rounded,
        [ThemeColors.of(context).blueCyan, ThemeColors.of(context).blueMaterial],
        '+${_dadosAtuais['crescimento']}%',
      ),
      _buildSummaryCard(context, 'Ticket Mãdio',
        'R\$ ${_dadosAtuais['ticketMedio'].toStringAsFixed(2)}',
        Icons.receipt_long_rounded,
        [ThemeColors.of(context).blueCyan, ThemeColors.of(context).blueLight],
        '+3.2%',
      ),
      _buildSummaryCard(context, 'Categorias',
        '${_categorias.length}',
        Icons.category_rounded,
        [ThemeColors.of(context).blueCyan, ThemeColors.of(context).blueMaterial],
        '100%',
          ...
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [ThemeColors.of(context).blueCyan, ThemeColors.of(context).blueMaterial],
                  ),
                  borderRadius: BorderRadius.circular(
                    isMobile ? 10 : 12,
                  ),
  ...
  Widget _buildDistributionBar(BuildContext context, Map<String, dynamic> categoria) {
    return Container(
  ...
  Widget _buildTopCategoriesCard(BuildContext context) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
  ...
  Widget _buildRankingItem(BuildContext context, Map<String, dynamic> categoria, int posicao) {
    final isMobile = ResponsiveHelper.isMobile(context);
  ...
  Widget _buildCategoryDetailCard(BuildContext context, Map<String, dynamic> categoria, int index) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      Expanded(
                        child: _buildMetricBox(context, 'Vendas',
                          '${categoria['vendas']}',
                          Icons.shopping_cart_rounded,
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildMetricBox(context, 'Ticket Mãdio',
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: categoria['crescimento'] > 0
                            ? ThemeColors.of(context).greenMaterial.withValues(alpha: 0.4)
                            : ThemeColors.of(context).redMain.withValues(alpha: 0.4),
                      ),
                    ),
                    child: Row(
                      ...
                      children: [
                        Icon(
                          ...
                          color: categoria['crescimento'] > 0
                              ? ThemeColors.of(context).greenMaterial.withValues(alpha: 0.8)
                              : ThemeColors.of(context).redMain.withValues(alpha: 0.8),
                          size: 20,
                        ),
                        const SizedBox(width: 8),
                        Text(
  ...
  Widget _buildGrowthRankingCard(BuildContext context) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
  ...
  Widget _buildGrowthItem(BuildContext context, Map<String, dynamic> categoria) {
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isPositive ? ThemeColors.of(context).greenMaterial : ThemeColors.of(context).redMain,
              borderRadius: BorderRadius.circular(8),
            ),
            child: Row(
              children: [
  ...
  void _exportarRelatorio() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        icon: Icon(Icons.check_rounded, color: ThemeColors.of(context).greenMaterial, size: 48),
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ListTile(
              leading: Icon(Icons.check_rounded, color: ThemeColors.of(context).redMain),
              ...
              onTap: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Gerando relatãrio PDF...'),
                    backgroundColor: ThemeColors.of(context).greenMaterial,
            ...
            ListTile(
              leading: Icon(Icons.table_chart_rounded, color: ThemeColors.of(context).greenMaterial),
