mport 'package:tagbean/design_system/design_system.dart';
...
/// Reposit�rio para dados do Dashboard
/// Combina dados de m�ltiplos endpoints do backend
class DashboardRepository {
  ...
  /// Obt�m estat�sticas de estrat�gias (da loja)
  /// GET /api/strategies/store/:storeId + agrega��o
  Future<ApiResponse<StrategiesStats>> getStrategiesStats(String storeId) async {
    try {
      // Busca estrat�gias da loja
      ...
      if (response.isSuccess && response.data != null) {
        ...
        for (final strategy in activeStrategies) {
          ...
          totalTodayGain += gain / 30; // Estimativa di�ria
        ...
        // Calcula crescimento baseado em ganho di�rio vs mensal
        // Nota: Quando dispon�vel, usar endpoint GET /api/dashboard/stats para fornecer crescimento real
        // Por enquanto, calcula propor��o entre ganho di�rio e mensal estimado
      ...
      return ApiResponse.error(response.errorMessage ?? 'Erro ao buscar estrat�gias');
    } catch (e) {
      return ApiResponse.error('Erro ao calcular estat�sticas: $e');
    }
  }

  /// Gera alertas baseado nos dados
  /// Combina informa��es de m�ltiplos endpoints
      ...
      if (productsResponse.isSuccess && productsResponse.data != null) {
        ...
        if (noPrice > 0) {
          alerts.add(DashboardAlert(
            ...
            details: 'Produtos cadastrados sem pre�o de venda definido. Isso impede a sincroniza��o com as tags ESL.',
          ));
        }
        ...
        if (negativaMargin > 0) {
          alerts.add(DashboardAlert(
            ...
            details: 'Produtos com pre�o de venda menor que o custo. Ajuste urgente necess�rio.',
          ));
        }
      }
      ...
      if (tagsResponse.isSuccess && tagsResponse.data != null) {
        ...
        if (offlineTags > 0) {
          alerts.add(DashboardAlert(
            ...
            details: 'Tags ESL n�o est�o respondendo h� mais de 24 horas. Verifique a conex�o e bateria.',
          ));
        }
  ...
  Future<ApiResponse<DashboardData>> loadDashboardData(String storeId) async {
    try {
      // Executa todas as requisi��es em paralelo
      ...
      // Constr�i o DashboardData com os resultados
  ...
  /// Retorna cor baseada no tipo de estrat�gia
