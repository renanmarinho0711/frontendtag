...
class _DashboardScreenState extends ConsumerState<DashboardScreen> with TickerProviderStateMixin {
  ...
  int _rebuildCounter = 0; // Contador para for�ar rebuild ao clicar no mesmo menu
  ...
  // OTIMIZA��O: Lista static const para evitar recria��o a cada rebuild
  ...
  // OTIMIZA��O: Reduzir c�lculos e chamadas MediaQuery
  void _scrollToSelectedItem() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final screenWidth = MediaQuery.sizeOf(context).width; // OTIMIZA��O: sizeOf � mais r�pido
      ...
      if (isFirstThree) {
        // Clicou nos primeiros 3 ? rolar para o in�cio (mostrando at� 3 menus)
        targetScroll = 0.0;
      } else if (isLastThree) {
        ...
        // Clicou no meio ? centralizar o item ou rolar para deixar vis�vel
        ...
        if (!isFullyVisible) {
          if (itemPosition < currentScroll) {
            // Item � esquerda ? rolar para tr�s 3 menus
  ...
  // Keys para for�ar rebuild quando clicar no mesmo menu
  ...
  // OTIMIZA��O REMOVIDA: N�o recriar GlobalKeys (causava rebuilds completos das telas)
  ...
  // Callbacks para os widgets de navega��o
  void _onItemSelected(int index, {bool fromDrawer = false}) {
    if (index == _selectedIndex) {
      // Clicou no mesmo menu - for�a rebuild para voltar ao dashboard do m�dulo
      setState(() => _rebuildCounter++);
    } else {
      setState(() => _selectedIndex = index);
    }
    // Reinicia anima��o de fade
  ...
  void _onNotificationTap() {
    // Navega para tela de notifica��es (index 9 = Settings > Notifica��es)
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('Notifica��es em breve'),
  ...
  void _onLogout() {
    // Mostra di�logo de confirma��o de logout
    showDialog(
      context: context,
  ...
  void _onProfileTap() {
    // Navega para tela de perfil/configura��es
    _onItemSelected(9); // Index 9 = Configura��es
  }
  ...
  // ============================================================================
  // M�TODOS DE NAVEGA��O REMOVIDOS - Agora usamos widgets extra�dos:
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // ?? SELETOR DE LOJA (apenas se usu�rio tem m�ltiplas lojas)
          StoreSwitcherCard(
            onStoreChanged: () {
          ...
          AdminPanelCard(
            onGerenciarClientes: () => _navigateTo(9), // Configura��es (com placeholder de clientes)
            onGerenciarLojas: () => _navigateTo(9), // Configura��es (com placeholder de lojas)
            onGerenciarUsuarios: () => _navigateTo(9), // Configura��es ? Usu�rios
            onVerConfiguracoes: () => _navigateTo(9), // Configura��es
          ),
          SizedBox(height: spacing),
          
          // ?? PRIMEIROS PASSOS (para usu�rios novos - substitui atalhos de teclado)
          OnboardingStepsCard(
            onImportProducts: () => _navigateTo(7), // Importa��o
            onRegisterTags: () => _navigateTo(2), // Etiquetas
            onBindTags: () => _navigateTo(2), // Etiquetas
            onActivateStrategy: () => _navigateTo(3), // Estrat�gias
            onConfigureStore: () => _navigateTo(9), // Configura��es
          ),
          SizedBox(height: spacing),
          
          // ?? PR�XIMA A��O RECOMENDADA (IA sugere o que fazer)
          NextActionCard(
          ...
          // ?? ATIVIDADE RECENTE (hist�rico do que aconteceu)
          SizedBox(height: spacing),
  ...
  Widget _buildDesktopLayout(double spacing, bool isTablet) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        // Primeira linha: Resumo + A��es (altura uniforme)
        IntrinsicHeight(
          child: Row(
        ...
        // 6?? ESTRAT�GIAS ATIVAS - largura total
        EstrategiasAtivasCard(
          onGerenciar: () => _navigateTo(3),
        ),
      ],
  ...
  /// Dialog para aplicar sugest�es automaticamente
  void _showAplicarSugestoesDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Aplicar Sugest�es da IA'),
        content: const Text(
          'Deseja aplicar automaticamente todas as sugest�es de ajuste de pre�o?\n\n'
          'Esta a��o ir� atualizar os pre�os de todos os produtos recomendados.',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              // TODO: Implementar aplica��o de sugest�es
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Row(
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Expanded(child: Text('Sugest�es aplicadas com sucesso!')),
                    ],
                  ),
  ...
  Widget _buildCompactMetricsGrid(BuildContext context) {
    ...
    final estatisticas = [
      ...
      {'label': 'Sincroniza��es', 'valor': '42', 'icon': Icons.sync_rounded, 'cor': ThemeColors.of(context).blueCyan, 'mudanca': '+3', 'tipo': 'aumento'},
  ...
  Widget _buildCompactMetricCard(BuildContext context, 
    ...
    // OTIMIZA��O: RepaintBoundary para isolar repaints
    return RepaintBoundary(
      child: Container(
        decoration: BoxDecoration(
          color: ThemeColors.of(context).surface,
          ...
          // OTIMIZA��O: Reduzir blur de sombra para melhor performance
          boxShadow: [
            BoxShadow(
  ...
  void _forcarSincronizacao() {
    ...
    // TODO: Implementar chamada real de sincroniza��o
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            _buildStatusRow(context, '�ltima sincroniza��o', 'H� 5 minutos', ThemeColors.of(context).blueMain),
            const SizedBox(height: 8),
  ...
  void _showFluxosPendentesDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Aqui voc� pode ver todos os fluxos inteligentes que precisam de sua aten��o.',
          textAlign: TextAlign.center,
        ),
        actions: [
          TextButton(
  ...
  // Fun��es auxiliares para determinar cores dos cards de alerta baseado na cor do alerta
  ...
  void _navegarParaDetalhesAlerta(Map<String, dynamic> alerta) {
    ...
    if (tipo.contains('sem pre�o') || tipo.contains('sem_preco')) {
      // Navegar para produtos sem pre�o
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              const SizedBox(width: 12),
              const Text('Filtro aplicado: Produtos sem pre�o'),
            ],
          ),
      ...
      // Navegar para precifica��o
      setState(() => _selectedIndex = 5); // Precifica��o
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          subtitle: Text(
            isProduto ? r['codigo'] ?? '' : r['produto'] ?? 'Sem v�nculo',
            style: TextStyle(
              fontSize: 12,
              color: ThemeColors.of(context).textSecondary,
            ),
