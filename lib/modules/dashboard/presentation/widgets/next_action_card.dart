...
/// Card de Próxima Ação Sugerida - IA sugere o prãximo passo baseado no estado do sistema
/// Lãgica de prioridade:
...
/// 6. Nenhuma estratãgia ? "Ative Estratégias de precificação"
class NextActionCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Se estã carregando, tem erro ou foi "dismiss", não mostrar
    if (dashboardState.isLoading || dashboardState.hasError) {
      return const SizedBox.shrink();
    }
    
    // Verifica se foi dismissed (reseta apãs 1 hora)
  ...
  Map<String, dynamic>? _determineNextAction(DashboardData data) {
    ...
    if (data.storeStats.tagsCount == 0) {
      return {
        ...
        'description': 'Nenhuma etiqueta eletrãnica foi cadastrada ainda. '
    ...
    if (offlineCount > 0) {
      return {
        ...
        'description': '$offlineCount tags estão sem Comunicação. '
            'Isso pode indicar problema de bateria ou distãncia do gateway.',
