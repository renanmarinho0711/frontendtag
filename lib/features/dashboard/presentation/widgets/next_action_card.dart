...
/// Card de Prxima Ao Sugerida - IA sugere o prximo passo baseado no estado do sistema
/// Lgica de prioridade:
...
/// 5. Produtos sem preo ? "Defina preos"
/// 6. Nenhuma estratgia ? "Ative estratgias de precificao"
class NextActionCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Se est carregando, tem erro ou foi "dismiss", no mostrar
    if (dashboardState.isLoading || dashboardState.hasError) {
      return const SizedBox.shrink();
    }
    
    // Verifica se foi dismissed (reseta aps 1 hora)
    if (isDismissed && !dismissNotifier.shouldShow) {
      return const SizedBox.shrink();
    }
    
    final data = dashboardState.data;
    final suggestion = _determineNextAction(context, data);
    
    // Se no houver sugesto, no mostrar o card
  ...
  Map<String, dynamic>? _determineNextAction(BuildContext context, DashboardData data) {
    // 1. Sem produtos
    if (data.storeStats.productsCount == 0) {
      return {
        'title': 'Importe seus produtos',
        'description': 'Voc ainda no tem produtos cadastrados. '
            'Importe do ERP ou cadastre manualmente para comear a usar o sistema.',
    ...
    if (data.storeStats.tagsCount == 0) {
      return {
        ...
        'description': 'Nenhuma etiqueta eletrnica foi cadastrada ainda. '
            'Adicione as tags ESL para exibir preos automaticamente nas prateleiras.',
    ...
    if (productsWithoutTag > 0 && bindingRate < 100) {
      return {
        ...
        'description': 'Voc tem $productsWithoutTag produtos sem tag vinculada. '
            'Vincule tags ESL para exibir preos automaticamente.',
    ...
    // 4. Tags offline (tags no vinculadas podem indicar offline)
    final offlineCount = data.storeStats.availableTagsCount;
    if (offlineCount > 0) {
      return {
        'title': 'Verifique as tags offline',
        'description': '$offlineCount tags esto sem comunicao. '
            'Isso pode indicar problema de bateria ou distncia do gateway.',
    ...
    if (productsWithoutPrice > 0) {
      return {
        'title': 'Defina preos dos produtos',
        'description': 'Voc tem $productsWithoutPrice produtos sem preo definido. '
            'Produtos sem preo no sero exibidos nas etiquetas.',
        'icon': Icons.money_off_rounded,
        'color': ThemeColors.of(context).warningDark,
        'buttonText': 'Definir Preos',
        'buttonIcon': Icons.attach_money_rounded,
        'onMainAction': onSetPrices,
      };
    }
    
    // 6. Nenhuma estratgia ativa
    if (data.strategiesStats.activeCount == 0) {
      return {
        'title': 'Ative estratgias de lucro',
        'description': 'Nenhuma estratgia de precificao est ativa. '
            'Ative estratgias para otimizar seus preos automaticamente.',
        'icon': Icons.auto_awesome_rounded,
        'color': ThemeColors.of(context).greenMaterial,
        'buttonText': 'Ativar Estratgia',
        'buttonIcon': Icons.play_arrow_rounded,
        'onMainAction': onActivateStrategy,
      };
    }
    
    // Tudo OK - no mostrar o card
    return null;
  }
}
