...
import 'dart:async';
import 'dart:convert';
// import 'package:file_picker/file_picker.dart'; // Não disponível nas dependências
...
/// Tela de importao em massa de produtos
...
class _ProdutosImportarScreenState extends ConsumerState<ProdutosImportarScreen> 
  ...
  Widget _buildHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: AppGradients.blueCyan(context),
        ...
        children: [
          Row(
            children: [
              ...
              _buildStepIndicator(4, 'Concludo', _step >= 4),
            ],
          ),
          if (_step == 3) ...[
    ...
    return Column(
      children: [
        Container(
          ...
          height: size,
          decoration: BoxDecoration(
            gradient: completed ? AppGradients.blueCyan(context) : null,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: AppSpacing.sm),
          _buildInfoItem('codigo', 'Cdigo de barras (mnimo 8 dgitos)'),
          _buildInfoItem('nome', 'Nome do produto (obrigatrio)'),
          _buildInfoItem('preco', 'Preo unitrio (formato: 9.99)'),
          _buildInfoItem('categoria', 'Categoria existente no sistema'),
          _buildInfoItem('estoque', 'Quantidade em estoque (opcional)'),
          _buildInfoItem('descricao', 'Descrio do produto (opcional)'),
        ],
      ),
    );
  }
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              const Icon(Icons.description_rounded, color: ThemeColors.of(context).blueCyan),
              SizedBox(width: AppSpacing.md),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              _buildFieldChip('Cdigo', item.codigo),
              _buildFieldChip('Preo', 'R\$ ${item.preco}'),
          ...
          children: [
            Expanded(
              child: OutlinedButton.icon(
                ...
                icon: const Icon(Icons.download_rounded),
                label: const Text('Baixar Relatrio'),
                style: OutlinedButton.styleFrom(
                  padding: EdgeInsets.symmetric(
                    vertical: AppSizes.paddingLg.get(isMobile, isTablet),
                  ),
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      ...
                      action: SnackBarAction(
                        ...
                        onPressed: () {
                          // Navega para a tela de vinculao em lote
  ...
  void _selecionarArquivo() async {
    ...
    // TODO: Implementar file_picker quando dependência estiver disponível
    /*
    try {
      ...
      if (file.bytes == null) {
        _showErrorSnackBar('No foi possvel ler o arquivo');
        return;
      }
      ...
      if (file.extension?.toLowerCase() == 'csv') {
        ...
        // Para Excel, por enquanto s suportamos CSV
        _showErrorSnackBar('Por enquanto, apenas arquivos CSV so suportados');
        return;
      }
      
      if (records.isEmpty) {
        _showErrorSnackBar('O arquivo no contm dados vlidos');
        return;
      }
      
      // Processa dados atravs do provider
