...
class SyncNotifier extends StateNotifier<SyncState> {
  ...
  /// Obtém estatísticas Minew da loja
  Future<MinewStoreStats?> getMinewStats(String storeId) async {
    try {
      final response = await _repository.getMinewStoreStats(storeId);
      if (response.isSuccess && response.data != null) {
        return response.data;
      }
      return null;
  ...
  /// Sincronização completa com Minew Cloud (tags + gateways + stats)
  Future<StoreSyncSummary?> syncMinewComplete(String storeId) async {
    if (!state.canSync) return null;
    ...
    try {
      final response = await _repository.syncMinewStoreComplete(storeId);
  ...
  /// Sincroniza apenas tags com Minew Cloud
  Future<StatsSyncResult?> syncMinewTags(String storeId) async {
    if (!state.canSync) return null;
    ...
    try {
      final response = await _repository.syncMinewTags(storeId);
  ...
  Future<StatsSyncResult?> importMinewTags(String storeId) async {
    if (!state.canSync) return null;

    state = state.copyWith(status: SyncModuleStatus.syncing);

    try {
      final response = await _repository.importMinewTags(storeId);
      ...
      if (response.isSuccess && response.data != null) {
        ...
        state = SyncState.error(
          response.errorMessage ?? 'Erro ao importar tags',
          history: state.history,
          settings: state.settings,
        );
        return null;
  ...
  /// Importa novos gateways da Minew Cloud
  Future<StatsSyncResult?> importMinewGateways(String storeId) async {
    if (!state.canSync) return null;

    state = state.copyWith(status: SyncModuleStatus.syncing);

    try {
      final response = await _repository.importMinewGateways(storeId);
      ...
      if (response.isSuccess && response.data != null) {
        ...
        state = SyncState.error(
          response.errorMessage ?? 'Erro ao importar gateways',
          history: state.history,
          settings: state.settings,
        );
        return null;
...
// ==========================================================================
// NOVOS PROVIDERS - MINEW STATS
// ==========================================================================

/// Provider para estatísticas Minew de uma loja específica
final minewStatsProvider = FutureProvider.family<MinewStoreStats?, String>((ref, storeId) async {
  final syncNotifier = ref.watch(syncProvider.notifier);
  return await syncNotifier.getMinewStats(storeId);
});

/// Provider para forçar refresh das stats Minew
final minewStatsRefreshProvider = StateProvider<int>((ref) => 0);

/// Provider combinado que obtém stats e observa refresh
final minewStoreStatsProvider = FutureProvider.family<MinewStoreStats?, String>((ref, storeId) async {
  // Observa o refresh counter para reagir a mudanças
  ref.watch(minewStatsRefreshProvider);
