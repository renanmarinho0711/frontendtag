...
class PricingAdjustmentState {
  ...
  factory PricingAdjustmentState.initial() => PricingAdjustmentState(
        config: const PricingAdjustmentConfigModel(),
      );
}
    ...
    try {
      ...
      if (response.isSuccess && response.data != null) {
        ...
        final allCritical = criticalResponse.isSuccess 
            ? (criticalResponse.data ?? []).map((p) => MarginReviewModel.fromJson(p as Map<String, dynamic>)).toList()
            : <MarginReviewModel>[];
    ...
    try {
      ...
      if (response.isSuccess && response.data != null) {
        final suggestions = response.data!
            .map((s) => AiSuggestionModel.fromJson(s as Map<String, dynamic>))
            .toList();
    ...
    try {
      ...
      if (response.isSuccess && response.data != null) {
        final history = response.data!
            .map((h) => PricingHistoryModel.fromJson(h as Map<String, dynamic>))
            .toList();
...
class DynamicPricingState {
  ...
  factory DynamicPricingState.initial() => DynamicPricingState(
...
/// Provider de reviṣo de margens
