    ...
    try {
      ...
      if (response.isSuccess && response.data != null) {
        ...
        state = state.copyWith(
          ...
          apiKey: (((data['apiKey']).toString()).toString()).toString() ?? data['chaveApi'] ?? state.apiKey,
          city: (((data['city']).toString()).toString()).toString() ?? data['cidade'] ?? state.city,
          // ignore: argument_type_not_assignable
          currentTemperature: (data['currentTemperature'] ?? data['temperaturaAtual'] ?? state.currentTemperature).toDouble(),
          currentCondition: (((data['currentCondition']).toString()).toString()).toString() ?? data['condicaoAtual'] ?? state.currentCondition,
          selectedFrequency: (((data['selectedFrequency']).toString()).toString()).toString() ?? data['frequenciaSelecionada'] ?? state.selectedFrequency,
        );
      } else {
