...
class HeatmapNotifier extends StateNotifier<HeatmapState> {
  ...
  Future<void> loadFromBackend() async {
    ...
    try {
      ...
      if (response.isSuccess && response.data != null) {
        ...
        if (zonasList is List) {
          for (final z in zonasList) {
            zonas.add(HeatmapZoneModel(
              id: z['id']?.toString() ?? '',
              nome: z['nome'] ?? z['name'] ?? '',
              trafego: z['trafego'] ?? z['fluxo'] ?? z['flow'] ?? 0,
              status: z['status']?.toString() ?? '',
              corKey: _parseColorKey(z['cor'] ?? z['color']) ?? 'blueMain',
              icone: Icons.map,
              tags: z['tags'] ?? 0,
              variacao: z['variacao'] ?? z['variation'] ?? 0,
              pessoas: z['pessoas'] ?? z['people'] ?? 0,
            ));
          }
        }
        ...
        // Sem dados no backend - mant�m estado vazio
  ...
  /// Parse semantic color key instead of Color object
  String _parseColorKey(dynamic colorValue) {
    if (colorValue == null) return 'textLight';
    if (colorValue is String) {
      // Check if it's already a semantic key
      if (colorValue.contains('#') || colorValue.startsWith('0x')) {
        return 'primary'; // Default to primary if it looks like hex
      }
      return colorValue; // Assume it's already a color key
    }
    return 'textLight';
  }
  ...
  Future<void> salvarConfiguracoes() async {
    ...
    try {
      // Encontra a estratgia de mapa de calor e atualiza sua configura��o
      ...
      if (strategies.isSuccess && strategies.data != null) {
        final heatmapStrategy = strategies.data!.firstWhere(
          ...
          orElse: () => throw Exception('Estratgia de mapa de calor n�o encontrada'),
        );
    ...
    try {
      ...
      if (strategies.isSuccess && strategies.data != null) {
        final rankingStrategy = strategies.data!.firstWhere(
          ...
          orElse: () => throw Exception('Estratgia de ranking n�o encontrada'),
        );
...
/// Estado da estratgia de Promo��es Relmpago
        ...
        if (promosList is List) {
          for (final p in promosList) {
            promocoes.add(FlashPromoModel(
              ...
              corKey: 'error', // Red color key for flash promos
    ...
    try {
      ...
      if (strategies.isSuccess && strategies.data != null) {
        final promoStrategy = strategies.data!.firstWhere(
          ...
          orElse: () => throw Exception('Estratgia de promo��es n�o encontrada'),
        );
        ...
        if (rotasList is List) {
          for (final r in rotasList) {
            rotas.add(SmartRouteModel(
              id: r['id']?.toString() ?? '',
              numero: r['numero']?.toString() ?? r['number']?.toString() ?? r['nome']?.toString() ?? r['name']?.toString() ?? '',
              ...
              corKey: 'success', // Green color key for completed routes
...
/// Provider para Promo��es Relmpago
