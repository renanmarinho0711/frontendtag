...
class HolidayEventModel {
  ...
  /// Obtém a cor dinâmica do evento (requer BuildContext)
  /// Use este método em widgets ao invés da propriedade [color]
  Color dynamicColor(BuildContext context) {
    final colors = ThemeColors.of(context);
    if (isActive) {
      if (isPositiveAdjustment) return colors.success;
      if (isNegativeAdjustment) return colors.error;
...
class LongHolidayModel {
  ...
  /// Obtém a cor dinâmica do feriado (requer BuildContext)
  Color dynamicColor(BuildContext context) {
    final colors = ThemeColors.of(context);
    if (isActive) {
      if (adjustment > 0) return colors.success;
      if (adjustment < 0) return colors.error;
      return colors.primary;
    }
    return colors.grey500;
  }
...
class SalaryCycleModel {
  ...
  /// Obtém a cor dinâmica do ciclo (requer BuildContext)
  Color dynamicColor(BuildContext context) {
    final colors = ThemeColors.of(context);
    if (isActive) {
      if (adjustment > 0) return colors.success;
  ...
  factory SalaryCycleModel.fromJson(Map<String, dynamic> json) {
    return SalaryCycleModel(
      ...
      color: StrategyModel.parseColor(json['cor'] ?? json['color']) ?? const Color(0xFF4CAF50),
      description: json['descricao'] as String? ?? json['description'] as String? ?? '',
    );
  }
...
class SalaryAdjustmentHistoryModel {
  ...
  /// Obtém a cor dinâmica baseada no ajuste (requer BuildContext)
  Color dynamicColor(BuildContext context) {
    final colors = ThemeColors.of(context);
    if (isPositiveAdjustment) return colors.success;
    if (isNegativeAdjustment) return colors.error;
  ...
  factory SalaryAdjustmentHistoryModel.fromJson(Map<String, dynamic> json) {
    return SalaryAdjustmentHistoryModel(
      ...
      color: StrategyModel.parseColor(json['color']) ?? const Color(0xFF4CAF50),
    );
  }
}
