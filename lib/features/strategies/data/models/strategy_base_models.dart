...
enum StrategyCategory {
  ...
  crossSelling('Cross-Selling', Icons.compare_arrows_rounded, 'brownMain'),
  visual('Visual', Icons.visibility_rounded, 'blueCyan');

  const StrategyCategory(this.label, this.icon, this.colorKey);
  final String label;
  final IconData icon;
  final String colorKey; // Semantic color key, resolved at runtime in UI
...
enum StrategyStatus {
  ...
  inactive('inativa', 'Inativa', 'textLight', Icons.cancel_rounded),
  draft('rascunho', 'Rascunho', 'textSecondary', Icons.edit_rounded),
  scheduled('agendada', 'Agendada', 'blueMain', Icons.schedule_rounded),
  executing('executando', 'Executando', 'orangeMain', Icons.sync_rounded);

  const StrategyStatus(this.value, this.label, this.colorKey, this.icon);
  final String value;
  final String label;
  final String colorKey; // Semantic color key, resolved at runtime
  final IconData icon;
...
enum ExecutionStatus {
  ...
  const ExecutionStatus(this.value, this.label, this.colorKey);
  final String value;
  final String label;
  final String colorKey; // Semantic color key, resolved at runtime
}
...
class StrategyModel {
  ...
  factory StrategyModel.fromJson(Map<String, dynamic> json) {
    return StrategyModel(
      ...
      primaryColorKey: _parseColorKey(json['cor'] ?? json['color']),
  ...
  /// Parse color to semantic color key
  static String _parseColorKey(dynamic color) {
    if (color is String) {
      return color; // Already a color key
    }
    if (color is int) {
      // Convert hex to a known color key or return default
      return 'primary';
    }
    return 'primary'; // Default color key
  }


  static List<String> _parseGradient(dynamic gradient) {
    // Gradients are now stored as color keys (semantic references)
    // that will be resolved at runtime in the UI layer
    if (gradient is List<String>) {
      return gradient;
    }
    return ['primary', 'primaryDark']; // Default gradient color keys
  }
...
class StrategyExecution {
  ...
  /// Status color key (resolved at runtime in UI)
  String get statusColorKey => isSuccess ? 'success' : 'error';
