...
class MarkdownRuleModel {
  ...
  factory MarkdownRuleModel.fromJson(Map<String, dynamic> json) {
    return MarkdownRuleModel(
      ...
      corKey: _parseColorKey(json['cor'] ?? json['color']) ?? 'greenMain',
...
class MarkdownProductModel {
  ...
  /// Retorna a chave de cor semântica baseada nos dias restantes
  /// Resolve para cor dinâmica no nível de UI
  String get statusColorKey {
    if (diasRestantes == 0) return 'error'; // Vermelho
    if (diasRestantes <= 2) return 'orangeDark';
    if (diasRestantes <= 4) return 'orangeMain';
    return 'greenMain';
  }


  /// Retorna o �cone baseado nos dias restantes
