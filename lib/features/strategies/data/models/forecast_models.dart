...
/// Tend�ncia de previs�o
enum ForecastTrend {
  alta('alta', 'Alta', 'success', Icons.trending_up_rounded),
  baixa('baixa', 'Baixa', 'error', Icons.trending_down_rounded),
  estavel('estavel', 'Estável', 'blueMain', Icons.trending_flat_rounded);

  const ForecastTrend(this.value, this.label, this.colorKey, this.icon);
  final String value;
  final String label;
...
class ForecastPredictionModel {
  ...
  factory ForecastPredictionModel.fromJson(Map<String, dynamic> json) {
    return ForecastPredictionModel(
      ...
      corKey: _parseColorKey(json['cor'] ?? json['color']),
