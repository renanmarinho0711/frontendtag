...
class _MapaCalorConfigScreenState extends ConsumerState<MapaCalorConfigScreen>
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('atenção Visual Inteligente', style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 12, mobileFontSize: 11, tabletFontSize: 11), overflow: TextOverflow.ellipsis, color: ThemeColors.of(context).textSecondary)),
              ],
            ),
          ),
  ...
  Widget _buildTabBar() {
    ...
    return Container(
      ...
      child: TabBar(
        ...
        tabs: [
          Tab(icon: Icon(Icons.settings_rounded, size: AppSizes.iconSmall.get(isMobile, isTablet)), text: 'ConfigurAção'),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('atenção Direcionada', style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 17, mobileFontSize: 15, tabletFontSize: 16), overflow: TextOverflow.ellipsis, fontWeight: FontWeight.bold, color: ThemeColors.of(context).surface, letterSpacing: -0.8)),
          ...
          Transform.scale(scale: 1.1, child: Switch(value: state.isStrategyActive, onChanged: (value) => ref.read(heatmapProvider.notifier).setStrategyActive(value), activeThumbColor: ThemeColors.of(context).surface, activeTrackColor: ThemeColors.of(context).surfaceOverlay50)),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('IntegrAção com Cãmeras', style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 15, mobileFontSize: 14, tabletFontSize: 14), overflow: TextOverflow.ellipsis, fontWeight: FontWeight.bold)),
                SizedBox(height: AppSizes.paddingXsAlt.get(isMobile, isTablet)),
                Text('Sistema detecta tráfego em tempo real via anãlise de vídeo com IA', style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 12, mobileFontSize: 11, tabletFontSize: 11), overflow: TextOverflow.ellipsis, height: 1.5)),
              ],
            ),
          ),
          SizedBox(width: AppSizes.paddingBase.get(isMobile, isTablet)),
      ...
      child: Column(
        ...
        children: [
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Expanded(child: Text('Parâmetros de AtivAção', style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 15, mobileFontSize: 14, tabletFontSize: 14), overflow: TextOverflow.ellipsis, fontWeight: FontWeight.bold, letterSpacing: -0.5))),
          ]),
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Container(padding: EdgeInsets.symmetric(horizontal: AppSizes.paddingBase.get(isMobile, isTablet), vertical: AppSizes.paddingXsAlt.get(isMobile, isTablet)), decoration: BoxDecoration(color: ThemeColors.of(context).orangeMain.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(AppSizes.paddingBase.get(isMobile, isTablet))), child: DropdownButton<String>(value: state.intensidadePiscar, underline: const SizedBox(), items: ['Baixa', 'Mãdia', 'Alta', 'Muito Alta'].map((e) => DropdownMenuItem(value: e, child: Text(e, style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 14, mobileFontSize: 13, tabletFontSize: 13), overflow: TextOverflow.ellipsis)))).toList(), onChanged: (value) => ref.read(heatmapProvider.notifier).setIntensidadePiscar(value!), style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 14, mobileFontSize: 13, tabletFontSize: 13), fontWeight: FontWeight.bold, color: ThemeColors.of(context).orangeMain))),
          ]),
          SizedBox(height: AppSizes.paddingXl.get(isMobile, isTablet)),
          Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.refresh_rounded, size: AppSizes.iconMediumLarge.get(isMobile, isTablet), color: ThemeColors.of(context).purpleMedium),
            SizedBox(width: AppSizes.paddingBase.get(isMobile, isTablet)),
            Expanded(child: Text('AtualizAção do Mapa', style: TextStyle(fontSize: ResponsiveHelper.getResponsiveFontSize(context, baseFontSize: 15, mobileFontSize: 14, tabletFontSize: 14), overflow: TextOverflow.ellipsis, fontWeight: FontWeight.w600))),
  ...
  Widget _buildOpcoesCard(HeatmapState state) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(height: AppSizes.paddingBase.get(isMobile, isTablet)),
        _buildSwitchOption('IntegrAção com Cãmeras', 'Usar anãlise de vídeo para detecção', Icons.videocam_rounded, state.integracaoCameras, (value) => ref.read(heatmapProvider.notifier).setIntegracaoCameras(value)),
      ]),
    );
  }
  ...
  Widget _buildZonaStat(IconData icon, String value, String label) {
    final isMobile = ResponsiveHelper.isMobile(context);
    final isTablet = ResponsiveHelper.isTablet(context);
    return Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: AppSizes.iconSmall.get(isMobile, isTablet), color: ThemeColors.of(context).textSecondary), const SizedBox(height: 4), Text(value, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)), Text(label, style: TextStyle(fontSize: 10, color: ThemeColors.of(context).textSecondary))]);
  }

  void _showInfoDialog() {
    showDialog(context: context, builder: (context) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), icon: Icon(Icons.check_rounded, color: ThemeColors.of(context).error, size: 56), title: const Text('Mapa de Calor'), content: const SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Direciona atenção para zonas frias da loja:', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)), SizedBox(height: 16), Text(' ESLs piscam em áreas pouco visitadas', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' IntegrAção com câmeras para detecção', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' Ajuste Automático de preços', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' Aumento de tráfego em zonas frias', style: TextStyle(fontSize: 13, height: 1.5))])), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Entendi'))]));
  }

  void _salvarConfiguracoes() async {
    await ref.read(heatmapProvider.notifier).salvarConfiguracoes();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [Icon(Icons.check_circle_rounded, color: ThemeColors.of(context).surface), const SizedBox(width: 12), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [Text('Configurações Salvas!', style: TextStyle(fontWeight: FontWeight.bold)), Text('Mapa de calor configurado', style: TextStyle(fontSize: 12))]))]), backgroundColor: ThemeColors.of(context).error, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))));
  }
}
