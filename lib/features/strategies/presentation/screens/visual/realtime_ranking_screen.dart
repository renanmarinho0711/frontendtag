...
class _RankingTempoRealConfigScreenState extends ConsumerState<RankingTempoRealConfigScreen>
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Ranking Tempo Real', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: -0.5)), Text('Gamificação nas ESLs', style: TextStyle(fontSize: 12, color: ThemeColors.of(context).textSecondary))])),
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(
        ...
        tabs: const [Tab(icon: Icon(Icons.settings_rounded, size: 18), text: 'ConfigurAção'), Tab(icon: Icon(Icons.emoji_events_rounded, size: 18), text: 'Top 5')],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Efeito Manada', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: ThemeColors.of(context).surface, letterSpacing: -0.8)), const SizedBox(height: 6), Text(state.isStrategyActive ? 'Posiãães exibidas nas ESLs' : 'Sistema inativo', style: TextStyle(fontSize: 13, color: ThemeColors.of(context).surfaceOverlay70))])),
          Transform.scale(scale: 1.1, child: Switch(value: state.isStrategyActive, onChanged: (value) => ref.read(realtimeRankingProvider.notifier).setStrategyActive(value), activeThumbColor: ThemeColors.of(context).surface, activeTrackColor: ThemeColors.of(context).surfaceOverlay50)),
        ],
      ),
    );
  }
  ...
  Widget _buildParametrosCard(RealtimeRankingState state) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: ThemeColors.of(context).blueMain.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)), child: Icon(Icons.tune_rounded, color: ThemeColors.of(context).blueMain, size: 22)), const SizedBox(width: 14), const Expanded(child: Text('Parâmetros de Exibição', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: -0.5)))]),
          const SizedBox(height: 24),
          Row(children: [Icon(Icons.refresh_rounded, size: 24, color: ThemeColors.of(context).blueMain), const SizedBox(width: 12), const Expanded(child: Text('Intervalo de AtualizAção', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600))), Container(padding: EdgeInsets.symmetric(horizontal: AppSizes.mediumPadding.get(isMobile, isTablet), vertical: 6), decoration: BoxDecoration(color: ThemeColors.of(context).blueMain.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)), child: Text('${state.intervaloAtualizacao} min', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: ThemeColors.of(context).blueMain)))]),
          Slider(value: state.intervaloAtualizacao.toDouble(), min: 15, max: 120, divisions: 21, label: '${state.intervaloAtualizacao} min', onChanged: (value) => ref.read(realtimeRankingProvider.notifier).setIntervaloAtualizacao(value.toInt()), activeColor: ThemeColors.of(context).blueMain),
          const SizedBox(height: 24),
          Row(children: [Icon(Icons.analytics_rounded, size: 24, color: ThemeColors.of(context).blueCyan), const SizedBox(width: 12), const Expanded(child: Text('Tipo de Ranking', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600))), Container(padding: EdgeInsets.symmetric(horizontal: AppSizes.mediumPadding.get(isMobile, isTablet), vertical: 6), decoration: BoxDecoration(color: ThemeColors.of(context).blueCyan.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)), child: DropdownButton<String>(value: state.tipoRanking, underline: const SizedBox(), items: ['Vendas', 'Faturamento', 'Quantidade', 'Margem'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (value) => ref.read(realtimeRankingProvider.notifier).setTipoRanking(value!), style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: ThemeColors.of(context).blueCyan)))]),
        ],
      ),
    );
  }
  ...
  Widget _buildOpcoesCard(RealtimeRankingState state) {
    return Container(
      ...
      child: Column(children: [
        _buildSwitchOption('Exibir Posição', 'Mostrar medalha de posição nas ESLs', Icons.emoji_events_rounded, state.exibirPosicao, (value) => ref.read(realtimeRankingProvider.notifier).setExibirPosicao(value)),
        const SizedBox(height: 12),
        _buildSwitchOption('AnimAção de Subida', 'Efeito visual quando produto sobe no ranking', Icons.trending_up_rounded, state.animacaoSubida, (value) => ref.read(realtimeRankingProvider.notifier).setAnimacaoSubida(value)),
      ]),
    );
  }
  ...
  Widget _buildSwitchOption(String title, String subtitle, IconData icon, bool value, Function(bool) onChanged) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: value ? ThemeColors.of(context).amberMain.withValues(alpha: 0.1) : ThemeColors.of(context).textSecondary, borderRadius: BorderRadius.circular(12), border: Border.all(color: value ? ThemeColors.of(context).amberMain.withValues(alpha: 0.3) : ThemeColors.of(context).textSecondary)),
      child: Row(children: [
        ...
        Transform.scale(scale: 0.95, child: Switch(value: value, onChanged: onChanged, activeThumbColor: ThemeColors.of(context).warning)),
      ]),
    );
  }
  ...
  Widget _buildProdutoCard(RankingProductModel produto, int index) {
    return TweenAnimationBuilder(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: ThemeColors.of(context).surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: produto.cor.withValues(alpha: 0.3), width: 2), boxShadow: [BoxShadow(color: produto.cor.withValues(alpha: 0.2), blurRadius: 20, offset: const Offset(0, 6))]),
        child: Column(children: [
          Row(children: [
            Container(width: 60, height: 60, decoration: BoxDecoration(gradient: LinearGradient(colors: [produto.cor, produto.cor.withValues(alpha: 0.7)]), borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: produto.cor.withValues(alpha: 0.3), blurRadius: 12, offset: const Offset(0, 4))]), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(produto.icone, color: ThemeColors.of(context).surface, size: 24), const SizedBox(height: 2), Text('${produto.posicao}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: ThemeColors.of(context).surface))])),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(produto.nome, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: -0.3), maxLines: 1, overflow: TextOverflow.ellipsis), const SizedBox(height: 6), Row(children: [Icon(Icons.shopping_cart_rounded, size: 14, color: ThemeColors.of(context).textSecondary), const SizedBox(width: 4), Text('${produto.vendas} vendas hoje', style: TextStyle(fontSize: 12, color: ThemeColors.of(context).textSecondary))])])),
            if (produto.isRising || produto.isFalling)
              Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: (produto.isRising ? ThemeColors.of(context).greenMain : ThemeColors.of(context).error).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: (produto.isRising ? ThemeColors.of(context).greenMain : ThemeColors.of(context).error).withValues(alpha: 0.3), width: 2)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(produto.isRising ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded, size: 16, color: produto.isRising ? ThemeColors.of(context).greenMain : ThemeColors.of(context).error), const SizedBox(width: 4), Text('${produto.variacao.abs()}', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: produto.isRising ? ThemeColors.of(context).greenMain : ThemeColors.of(context).error))])),
          ]),
          const SizedBox(height: 12),
          Container(
            ...
            child: Row(children: [
              Expanded(child: Column(children: [Icon(Icons.attach_money_rounded, size: 20, color: produto.cor), const SizedBox(height: 4), Text(produto.faturamento, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: produto.cor)), const SizedBox(height: 2), Text('Faturamento', style: TextStyle(fontSize: 10, color: ThemeColors.of(context).textSecondary))])),
              Container(width: 1, height: 50, color: ThemeColors.of(context).textSecondary),
              Expanded(child: Column(children: [Icon(Icons.percent_rounded, size: 20, color: produto.cor), const SizedBox(height: 4), Text(produto.margemFormatted, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: produto.cor)), const SizedBox(height: 2), Text('Margem', style: TextStyle(fontSize: 10, color: ThemeColors.of(context).textSecondary))])),
            ]),
          ),
        ]),
      ),
  ...
  void _showInfoDialog() {
    showDialog(context: context, builder: (context) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), icon: Icon(Icons.check_rounded, color: ThemeColors.of(context).warning, size: 56), title: const Text('Ranking Tempo Real'), content: const SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Exibe posição de vendas nas etiquetas eletrônicas:', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)), SizedBox(height: 16), Text(' Ranking atualizado automaticamente', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' GamificAção aumenta o engajamento', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' Clientes veem produtos mais vendidos', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' Efeito manada impulsiona vendas', style: TextStyle(fontSize: 13, height: 1.5)), SizedBox(height: 8), Text(' Aumenta conversão em 15-25%', style: TextStyle(fontSize: 13, height: 1.5))])), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Entendi'))]));
  }

  void _salvarConfiguracoes() async {
    await ref.read(realtimeRankingProvider.notifier).salvarConfiguracoes();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [Icon(Icons.check_circle_rounded, color: ThemeColors.of(context).surface), const SizedBox(width: 12), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [Text('Configurações Salvas!', style: TextStyle(fontWeight: FontWeight.bold)), Text('Ranking tempo real configurado', style: TextStyle(fontSize: 12))]))]), backgroundColor: ThemeColors.of(context).warning, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))));
  }
}
