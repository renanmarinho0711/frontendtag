...
class ApiService {
  ...
  Future<bool> _refreshToken() async {
    ...
    try {
      ...
      final uri = _buildUri(ApiConstants.auth.refreshToken);
      ...
      if (response.statusCode == 200) {
        ...
        final newToken = (jsonData['accessToken'] ?? jsonData['token']) as String?;
  ...
  ApiResponse<T> _handleResponse<T>(
    ...
    if (statusCode >= 400 && statusCode < 500) {
      ...
      try {
        ...
        errorMessage = ((jsonData['message'] ?? jsonData['error']) ?? errorMessage) as String;
      ...
      // 401 Unauthorized - pode ser sessão expirada OU credenciais inválidas

      if (statusCode == 401) {

        // Verifica se é login (credenciais) ou sessão expirada
        ...
        // (login falho não deve limpar nada, pois não tinha sessão)
