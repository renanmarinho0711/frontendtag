...
class FeedbackService {
  ...
  void showWarning(String message, {Duration? duration}) {
    _showSnackBar(
      message: message,
      backgroundColor: AppThemeColors.warningDark,
      icon: Icons.warning_amber_outlined,
      duration: duration ?? const Duration(seconds: 3),
    );
  }

  /// Exibe um SnackBar de informa��o
  ...
  /// Exibe um SnackBar de carregamento (n�o fecha automaticamente)
  ScaffoldFeatureController<SnackBar, SnackBarClosedReason>? showLoading(
      String message) {
    return _showSnackBar(
      message: message,
      backgroundColor: AppThemeColors.grey800,
      icon: null,
      duration: const Duration(days: 1), // N�o fecha automaticamente
    ...
    final snackBar = SnackBar(
      content: Row(
        children: [
          ...
          if (icon != null) ...[
            Icon(icon, color: AppThemeColors.surface, size: 20),
            const SizedBox(width: 12),
          ],
          Expanded(
            child: Text(
              message,
              style: TextStyle(color: AppThemeColors.surface),
            ),
          ),
        ],
      ),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor:
                  isDangerous ? AppThemeColors.error : (confirmColor ?? AppThemeColors.primary),
