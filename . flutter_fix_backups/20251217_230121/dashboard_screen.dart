...
import 'package:tagbean/features/dashboard/presentation/widgets/estratégias_ativas_card.dart';
...
// Widgets de navegação extraãdos
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> with TickerProviderStateMixin {
  ...
  // ValueKey dinãmica usada em _getSelectedScreen() com _rebuildCounter
  // permite resetar módulo ao clicar no mesmo menu

  // OTIMIZAããO: Lista static const para evitar recriAção a cada rebuild
  ...
  void initState() {
    ...
    // Carrega os dados do dashboard apãs o primeiro frame
  ...
  // OTIMIZAããO: Reduzir cãlculos e chamadas MediaQuery
  void _scrollToSelectedItem() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final screenWidth = MediaQuery.sizeOf(context).width; // OTIMIZAããO: sizeOf é mais rápido
      ...
      if (isFirstThree) {
        // Clicou nos primeiros 3 ? rolar para o início (mostrando atã 3 menus)
        targetScroll = 0.0;
      } else if (isLastThree) {
        // Clicou nos últimos 3 ? rolar para o final (mostrando atã 3 menus)
  ...
  // OTIMIZAããO REMOVIDA: Não recriar GlobalKeys (causava rebuilds completos das telas)
  // GlobalKeys agora são final e persistem durante toda vida ãtil do widget
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // Drawer para mobile - usando widget extraãdo
            ...
            children: [
              ...
              Expanded(
                child: Row(
                  mainAxisSize: MainAxisSize.max,
                  children: [
                    // Navigation rail apenas para tablet e desktop - usando widget extraãdo
      ...
      // Bottom navigation para mobile - usando widget extraãdo
  ...
  void _onItemSelected(int index, {bool fromDrawer = false}) {
    if (index == _selectedIndex) {
      // Clicou no mesmo menu - forãa rebuild para voltar ao dashboard do módulo
  ...
  // ============================================================================
  // MãTODOS DE NAVEGAããO REMOVIDOS - Agora usamos widgets extraãdos:
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // ?? SELETOR DE LOJA (apenas se Usuário tem mãltiplas lojas)
          StoreSwitcherCard(
            onStoreChanged: () {
          ...
          // ?? PRãXIMA AããO RECOMENDADA (IA sugere o que fazer)
          NextActionCard(
          ...
          // 2?? ALERTAS ACIONãVEIS (sã aparece se houver alertas)
          ...
          // ?? BLOCO 4: FLUXOS INTELIGENTES (se houver pendãncias)
  ...
  /// Layout para dispositivos mãveis - tudo empilhado verticalmente
  ...
  Widget _buildDesktopLayout(double spacing, bool isTablet) {
    return Column(
      ...
      children: [
        ...
        // 6?? ESTRATãGIAS ATIVAS - largura total
        EstrategiasAtivasCard(
          onGerenciar: () => _navigateTo(3),
        ),
      ],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'TagBeans Store é ${_formatDate(DateTime.now())}',
                  style: TextStyle(
                    fontSize: ResponsiveHelper.getResponsiveFontSize(
                      context,
                      baseFontSize: 13,
  ...
  Widget _buildCompactMetricCard(BuildContext context, 
    ...
    // OTIMIZAããO: RepaintBoundary para isolar repaints
    return RepaintBoundary(
      child: Container(
        decoration: BoxDecoration(
          color: ThemeColors.of(context).surface,
          ...
          // OTIMIZAããO: Reduzir blur de sombra para melhor performance
          boxShadow: [
            BoxShadow(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${_estratégiasData['ativas']} Estratégias ativas',
                        style: TextStyle(
                          fontSize: ResponsiveHelper.getResponsiveFontSize(
                            context,
                            baseFontSize: 15,
                ...
                children: [
                  Expanded(
                    child: _buildGanhoInfo(context, 'Ganho Mensal',
                      ...
                      'R\$ ${_estratégiasData['ganho_mensal'].toStringAsFixed(2)}',
                      'em ${_estratégiasData['produtos_afetados']} produtos',
                    ),
                  ),
                  SizedBox(
                    width: ResponsiveHelper.getResponsivePadding(
                  ...
                  Expanded(
                    child: _buildGanhoInfo(context, 'Ganho Hoje',
                      Icons.today_rounded,
                      'R\$ ${_estratégiasData['ganho_hoje'].toStringAsFixed(2)}',
                      '+15% vs. ontem',
                    ),
                  ),
                ],
                        ...
                        ? Column(
                            ...
                            children: [
                              ...
                              ListView.builder(
                                ...
                                itemCount: (_estrategiasData['estratégias'] as List).length,
                                itemBuilder: (context, index) {
                                  final estrategia = (_estrategiasData['estratégias'] as List)[index];
                                  return RepaintBoundary(
                                    child: Padding(
  ...
  // NOTA: Mãtodo _buildRecentActivityCard() removido
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            _buildStatusRow(context, 'Última sincronização', 'Hã 5 minutos', ThemeColors.of(context).blueMain),
            const SizedBox(height: 8),
            _buildStatusRow(context, 'Pendãncias', '0 itens', ThemeColors.of(context).textSecondary),
          ],
        ),
        actions: [
          TextButton(
...
/// SearchDelegate para busca rãpida no TagBean
class _TagBeanSearchDelegate extends SearchDelegate<String?> {
  ...
  Widget _buildSearchResults(BuildContext context) {
    ...
    final resultados = <Map<String, dynamic>>[
      {'tipo': 'produto', 'nome': 'Coca-Cola 2L', 'código': '7891234567890', 'preco': 'R\$ 12,99'},
      {'tipo': 'produto', 'nome': 'Arroz Tio João 5kg', 'código': '7891000123456', 'preco': 'R\$ 29,90'},
      {'tipo': 'tag', 'mac': 'AA:BB:CC:DD:EE:01', 'status': 'Online', 'produto': 'Coca-Cola 2L'},
    ].where((r) {
      final q = query.toLowerCase();
      return r['nome']?.toString().toLowerCase().contains(q) == true ||
             r['código']?.toString().toLowerCase().contains(q) == true ||
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          title: Text(
            isProduto ? (((r['nome']).toString()).toString()).toString() ?? '' : r['mac'] ?? '',
            style: const TextStyle(fontWeight: FontWeight.w600),
          ),
          subtitle: Text(
            isProduto ? (((r['código']).toString()).toString()).toString() ?? '' : r['produto'] ?? 'Sem vãnculo',
            style: TextStyle(
              fontSize: 12,
              color: ThemeColors.of(context).textSecondary,
            ),
          ...
          onTap: () {
            close(context, (((r['nome']).toString()).toString()).toString() ?? r['mac']);
          },
        );
      },
    );
